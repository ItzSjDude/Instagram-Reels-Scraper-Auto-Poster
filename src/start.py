...
# Generator function to handle configuration setup steps
def configure():
...
def start_configuration():
    ...
    mainConfig.IS_REMOVE_FILES = yield st.text_input("Turn On File Remover After Posting? 1=On;0=Off :")
    Helper.save_config('IS_REMOVE_FILES', mainConfig.IS_REMOVE_FILES)
    if mainConfig.IS_REMOVE_FILES == "1":
        mainConfig.REMOVE_FILE_AFTER_MINS = yield st.number_input("Define the interval in minutes to remove uploaded files:")
    ...
    mainConfig.IS_ENABLED_REELS_SCRAPER = yield st.text_input("Turn On Reels Scraper? 1=On;0=Off :")
    Helper.save_config('IS_ENABLED_REELS_SCRAPER', mainConfig.IS_ENABLED_REELS_SCRAPER)
    if mainConfig.IS_ENABLED_REELS_SCRAPER == "1":
        mainConfig.FETCH_LIMIT = yield st.number_input("Screper fetch limit in number Ex. 50")
        Helper.save_config('FETCH_LIMIT', mainConfig.FETCH_LIMIT)
        mainConfig.SCRAPER_INTERVAL_IN_MIN = yield st.number_input("Reels scraping interval in minutes Ex. 120: For every 2 hours")
    ...
    mainConfig.IS_ENABLED_AUTO_POSTER = yield st.text_input("Turn On Reels Autoposter? 1=On;0=Off :")
    Helper.save_config('IS_ENABLED_AUTO_POSTER', mainConfig.IS_ENABLED_AUTO_POSTER)
    if mainConfig.IS_ENABLED_AUTO_POSTER == "1":
        mainConfig.POSTING_INTERVAL_IN_MIN = yield st.number_input("Reels posting interval in minutes Ex. 10: For every 10 minutes")
    ...
    mainConfig.IS_POST_TO_STORY = yield st.text_input("Turn On post reels into story? 1=On;0=Off :")
    ...
    mainConfig.USERNAME = yield st.text_input("Enter Instagram Username:")
    Helper.save_config('USERNAME', mainConfig.USERNAME)

    mainConfig.PASSWORD = yield st.text_input("Enter Instagram Password:", type="password")
    Helper.save_config('PASSWORD', mainConfig.PASSWORD)

    # Step 6: Additional Configuration
    st.write("Step 6: Additional Configuration")
    mainConfig.ACCOUNTS = yield st.text_input("Enter list of accounts which you want to scrape (comma separated):")
    Helper.save_config('ACCOUNTS', mainConfig.ACCOUNTS)

    mainConfig.HASTAGS = yield st.text_input("Enter hashtags which you want to add while posting:")
    Helper.save_config('HASTAGS', mainConfig.HASTAGS)

    mainConfig.LIKE_AND_VIEW_COUNTS_DISABLED = yield st.text_input("Like and view counts disabled? 1=Disabled;0=Enabled :")
    Helper.save_config('LIKE_AND_VIEW_COUNTS_DISABLED', mainConfig.LIKE_AND_VIEW_COUNTS_DISABLED)

    mainConfig.DISABLE_COMMENTS = yield st.text_input("Comments disabled? 1=Disabled;0=Enabled :")
    ...
    mainConfig.IS_ENABLED_YOUTUBE_SCRAPING = yield st.text_input("Turn On YouTube Shorts Scraper? 1=On;0=Off :")
    ...
    if mainConfig.IS_ENABLED_YOUTUBE_SCRAPING == "1":
        mainConfig.YOUTUBE_API_KEY = yield st.text_input("Enter Youtube API KEY:")
        Helper.save_config('YOUTUBE_API_KEY', mainConfig.YOUTUBE_API_KEY)
        mainConfig.CHANNEL_LINKS = yield st.text_input("Enter channel links to scrape (comma separated):")
