...
def configure():
    if st.checkbox("Start Configuration"):
        ...
        mainConfig.IS_REMOVE_FILES = st.select_slider("Turn On File Remover After Posting?", options=['0', '1'])
        ...
        mainConfig.IS_ENABLED_REELS_SCRAPER = st.select_slider("Turn On Reels Scraper?", options=['0', '1'])
        ...
        mainConfig.IS_ENABLED_AUTO_POSTER = st.select_slider("Turn On Reels Autoposter?", options=['0', '1'])
        ...
        mainConfig.IS_POST_TO_STORY = st.select_slider("Turn On post reels into story?", options=['0', '1'])
        ...
        mainConfig.LIKE_AND_VIEW_COUNTS_DISABLED = st.select_slider("Like and view counts disabled?", options=['0', '1'])
        Helper.save_config('LIKE_AND_VIEW_COUNTS_DISABLED', mainConfig.LIKE_AND_VIEW_COUNTS_DISABLED)

        mainConfig.DISABLE_COMMENTS = st.select_slider("Comments disabled?", options=['0', '1'])
        Helper.save_config('DISABLE_COMMENTS', mainConfig.DISABLE_COMMENTS)

        mainConfig.IS_ENABLED_YOUTUBE_SCRAPING = st.select_slider("Turn On YouTube Shorts Scraper?", options=['0', '1'])
