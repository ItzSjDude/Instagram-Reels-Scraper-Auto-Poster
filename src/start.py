...
def configure():
    ...
    st.header("Step 1: File Remover Configuration")
    mainConfig.IS_REMOVE_FILES = st.radio("Turn On File Remover After Posting? 1=On;0=Off :", ('1', '0'), index=0)
    ...
    st.header("Step 2: Reels Scraper Configuration")
    mainConfig.IS_ENABLED_REELS_SCRAPER = st.radio("Turn On Reels Scraper? 1=On;0=Off :", ('1', '0'), index=0)
    ...
    st.header("Step 3: Reels Autoposter Configuration")
    mainConfig.IS_ENABLED_AUTO_POSTER = st.radio("Turn On Reels Autoposter? 1=On;0=Off :", ('1', '0'), index=0)
    ...
    st.header("Step 4: Post Reels to Story Configuration")
    mainConfig.IS_POST_TO_STORY = st.radio("Turn On post reels into story? 1=On;0=Off :", ('1', '0'), index=0)
    Helper.save_config('IS_POST_TO_STORY', mainConfig.IS_POST_TO_STORY)

    # Step 5: Instagram Account Configuration
    st.header("Step 5: Instagram Account Configuration")
    ...
    st.header("Step 6: Additional Configuration")
    ...
    mainConfig.LIKE_AND_VIEW_COUNTS_DISABLED = st.radio("Like and view counts disabled? 1=Disabled;0=Enabled :", ('1', '0'), index=1)
    Helper.save_config('LIKE_AND_VIEW_COUNTS_DISABLED', mainConfig.LIKE_AND_VIEW_COUNTS_DISABLED)

    mainConfig.DISABLE_COMMENTS = st.radio("Comments disabled? 1=Disabled;0=Enabled :", ('1', '0'), index=1)
    Helper.save_config('DISABLE_COMMENTS', mainConfig.DISABLE_COMMENTS)

    # Step 7: YouTube Shorts Scraper Configuration
    st.header("Step 7: YouTube Shorts Scraper Configuration")
    mainConfig.IS_ENABLED_YOUTUBE_SCRAPING = st.radio("Turn On YouTube Shorts Scraper? 1=On;0=Off :", ('1', '0'), index=0)
